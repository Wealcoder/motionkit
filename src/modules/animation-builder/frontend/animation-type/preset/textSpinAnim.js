...
const PRESET_KEY = "wcf-mk-text-spin-fa";
...
export function textSpinAnim() {
  ...
  function teardown(id) {
    ...
    if (!inst) return;
    inst.timelines.forEach((tl) => { try { tl.kill(); } catch {} });
  ...
  function handler(e) {
    ...
    try {
      ...
      console.warn(
        `[textSpin] invalid itemClass "${itemClass}":`,
        err.message,
      );
      return;
    }
    ...
    elements.forEach((originalItem, idx) => {
      ...
      wrapper.style.display = isInline ? "inline-block" : cs.display || "inline-block";
      ...
      gsap.set(originalSplit.chars, { opacity: 1, rotationX: 0, transformPerspective: 600 });
      gsap.set(cloneSplit.chars, { opacity: 0, rotationX: -90, transformPerspective: 600 });
      ...
      requestAnimationFrame(() => requestAnimationFrame(() => {
        ...
        const buildScrollTriggerAnim = () => {
          ...
          tl.scrollTrigger = window.ScrollTrigger?.create({ ...stConfig, animation: tl });
          timelines.push(tl);
        };
        ...
        const attachHover = () => {
          ...
          triggers.forEach((trig) => {
            let tl = null;
            const onEnter = () => {
              if (tl) { try { tl.kill(); } catch {} }
              tl = gsap.timeline();
              buildSpinForward(tl, delay);
              timelines.push(tl);
            };
            const onLeave = () => { if (tl) tl.reverse(); };
        ...
        switch (triggerType) {
          case "on_scroll":        buildScrollTriggerAnim(); break;
          case "play_with_scroll": buildScrubbedAnim(); break;
          case "page_load":        buildPageLoad(); break;
